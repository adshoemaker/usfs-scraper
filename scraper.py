...
def parse_milestones(html: str) -> list[dict]:
    """
    Parse the Project Milestones table from a project detail page.
    Returns a list of {"milestone": str, "date": str, "estimated": bool}
    """
...
def fetch_milestones(session: requests.Session, project_url: str) -> list[dict]:
    """Fetch a project detail page and return its milestone list."""
    try:
        r = session.get(project_url, timeout=30)
        r.raise_for_status()
        return parse_milestones(r.text)
    except requests.RequestException as e:
        print(f"    !! Could not fetch milestones from {project_url}: {e}")
        return []
...
def scrape_forest(session: requests.Session, forest: dict) -> list[dict]:
    ...
    try:
        ...
        for _p in _existing.get("projects", []):
            if _p.get("milestones"):
                existing_milestones[_p["project_url"]] = _p["milestones"]
    except Exception:
        pass
    ...
    for p in milestone_projects:
        ...
        if do_full or is_new:
            ...
            # Re-use cached milestones for existing projects on non-full days
            p["milestones"] = existing_milestones[p["project_url"]]
    ...
    if to_fetch:
        ...
        for p in to_fetch:
            ...
            milestones = fetch_milestones(session, p["project_url"])
            p["milestones"] = milestones
            if milestones:
                print(f"    ✓ {p['project_name'][:50]} — {len(milestones)} milestones")
    else:
