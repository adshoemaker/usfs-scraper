...
from database import create_tables, upsert_project, mark_inactive_projects, print_summary, get_connection
...
def run_scraper():
    ...
    # ── Build JSON output including first_seen from the database ──
    # Look up first_seen for every project and include it in the JSON
    # so the hosted app (which doesn't have the .db file) can filter by date.
    conn = get_connection()
    url_to_first_seen = {}
