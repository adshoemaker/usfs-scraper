...
def filter_projects(projects, search="", forest_code="", status="", days=""):
    """Filter the project list by search, forest, status, and date added."""
    results = []
    search_lower = search.lower()

    # Calculate the cutoff date if a days filter is set
    cutoff = None
    if days:
    ...
    for p in projects:
        ...
        if cutoff:
            ...
            if not first_seen_str:
                continue
            try:
                # Parse the ISO date string, handle both tz-aware and naive
...
<div class="container">
    ...
    <form class="filters" method="GET" action="/">
        ...
        <div>
            ...
            {% if search or selected_forest or selected_status or selected_days %}
...
def index():
    ...
    # Anything added in the last 7 days gets a NEW badge automatically
    recent_cutoff = (
